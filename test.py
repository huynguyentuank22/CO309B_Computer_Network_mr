...
class PeerNetwork:
        ...
        while True:
            try:
                ...
                if (message['type'] == 'CONNECT_REQUEST' and 
                    ...
                    # Print new request notification
    ...
    def update_pending_requests(self, new_request: Dict):
        ...
        with self.request_lock:
            ...
            if existing_request:
                ...
                existing_request['strength'] += 1  # Increment strength for repeated requests
            ...
            while self.is_broadcasting and not self.is_connected:
                try:
                    request_msg = pickle.dumps({
                        ...
                        'sequence': broadcast_count  # Add sequence number for tracking
                    })
    ...
    def start(self):
        ...
        if not self.initialize_udp_socket():
            print("Failed to initialize UDP socket. Exiting...")
            return
        ...
        while True:
            if not self.is_connected:
                print("\nAvailable actions:")
                print("1. Broadcast connection request")
                print("2. List pending requests")
                print("3. Accept pending request")
                print("4. Just listen")
                print("5. Quit")
                ...
                if choice == '1':
                    ...
                    self.display_pending_requests()
                    username = input("Enter username to accept: ").strip()
                    if username:
                        self.accept_connection(username)
                elif choice == '4':
                    print("Listening for requests... (Press Enter to return to menu)")
                    input()
                elif choice == '5':
                    break
            else:
